const inquirer = require('inquirer');
const fs = require('fs');

// function is adding data to readme
function writeReadme(fileName, readmeData) {
    fs.writeFile(fileName, readmeData, function (err) {
        if (err) {
           return console.log(err)
        }
        console.log("Readme generated!")
    })
}


inquirer.prompt([
//title 
    {type: 'input',
    message: "What is the Title of your Project?",
    name: 'title',
    validate: title => {
        if(title.length < 5){
            return "Title is too short.";
        }
        else{
            // all validation checks passed
            return true;
        }
    }
},
//Description
    {type: 'input',
    message: "What is your Project Description?",
    name: 'description',
    validate: description => {
        if(description.length < 10){
            return "Description is too short.";
        }
        else{
            // all validation checks passed
            return true;
        }
    }
},
//Installation
    {type: 'input',
    message: "What are the steps for Installation?",
    name: 'installation',
    validate: installation => {
        if(installation.length < 5){
            return "Installation steps are required.";
        }
        else{
            // all validation checks passed
            return true;
        }
    }
},
//Usage
    {type: 'input',
    message: "What is the Usage description?",
    name: 'usage',
    validate: usage => {
        if(usage.length < 10){
            return "Usage is too short.";
        }
        else{
            // all validation checks passed
            return true;
        }
    }
},
//License
    {type: 'input',
    message: "What is the License?",
    name: 'license',
    validate: license => {
         if(license.length < 3){
             return "License required.";
    }
    else{
        // all validation checks passed
        return true;
    }
}
},
//Contributing
    {type: 'input',
    message: "Who are the contributors (github usernames)?",
    name: 'contributor',
    validate: contributor => {
         if(contributor.length < 3){
             return "Contributor(s) required.";
    }
    else{
        // all validation checks passed
        return true;
    }
}
},
//Tests
//optional
    {type: 'input',
    message: "What tests are going to be run?",
    name: 'test',

},
//Questions 
//optional
{type: 'input',
message: "Any Questions?",
name: 'question',

}


]).then( response => {
    console.log(response.title);
    // function to write README file
    let readmeData = "";
    readmeData += "# Table of Contents:"
    readmeData += "\n";
    readmeData += "## Title"
    readmeData += "\n";
    readmeData += "## Description"
    readmeData += "\n";
    readmeData += "## Installation Steps"
    readmeData += "\n";
    readmeData += "## Usage Description"
    readmeData += "\n";
    readmeData += "## License"
    readmeData += "\n";
    readmeData += "## Contributors"
    readmeData += "\n";
    readmeData += "## Tests"
    readmeData += "\n";
    readmeData += "## Questions"
    readmeData += "\n";
    readmeData += "\n";
    readmeData += "## Title"
    readmeData += "\n";
    readmeData += response.title; // data under heading
    readmeData += "\n";
    readmeData += "\n";
    readmeData += "## Description"
    readmeData += "\n";
    readmeData += response.description; // data under heading
    readmeData += "\n";
    readmeData += "\n";
    readmeData += "## Installation Steps"
    readmeData += "\n";
    readmeData += response.installation; // data under heading
    readmeData += "\n";
    readmeData += "\n";
    readmeData += "## Usage Description"
    readmeData += "\n";
    readmeData += response.usage; // data under heading
    readmeData += "\n";
    readmeData += "\n";
    readmeData += "## License"
    readmeData += "\n";
    readmeData += response.license; // data under heading
    readmeData += "\n";
    readmeData += "\n";
    readmeData += "## Contributors"
    readmeData += "\n";
    readmeData += response.contributor; // data under heading
    readmeData += "\n";
    readmeData += "\n";
    readmeData += "## Tests"
    readmeData += "\n";
    readmeData += response.test; // data under heading
    readmeData += "\n";
    readmeData += "\n";
    readmeData += "## Questions"
    readmeData += "\n";
    readmeData += response.question; // data under heading
    readmeData += "\n";
    

    writeReadme("index.md", readmeData);
});




// // function to initialize program
// function init() {

// }

// // function call to initialize program
// init();
